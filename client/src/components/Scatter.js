  ...
  render() {
    const drawScatter = (category) => {
      return this.props.stats.map((player) => (
        <circle
          style={{ fill: `${pickColor(player.name)}` }}
          cx={this.props.xScale(endOfDay(new Date(player.date)))}
          cy={this.props.yScale(player[category])}
          r="4"
          onMouseOver={(e) => {
            console.log(e);
          }}
        >
          <title>{`${player.name}: ${player[category]} ${category}, ${player.date}`}</title>
        </circle>
      ));
    };
